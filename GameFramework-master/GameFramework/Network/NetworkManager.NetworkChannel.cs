        ...
        private sealed partial class NetworkChannel : INetworkChannel, IDisposable
        {
            ...
            private bool Process()
            {
                ...
                {                    
                    ...
                    ushort packetLength = Utility.Converter.GetUshortFromBytes(m_ReceiveState.GetBuffer(), 2);
                ...
                {                    
                    ...
                    using (MemoryStream memoryStream = new MemoryStream(m_ReceiveState.GetBuffer(), 0, m_ReceiveState.Length, false))
                    {
                        lock (m_NetworkHelper)
                        {                 
                            Log.Info("数据长度为: " + memoryStream.Length);
